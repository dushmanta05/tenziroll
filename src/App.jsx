...
const App = () => {
  ...
  function rollDice() {
    if (!tenzies) {
      ...
      setDiceValue((oldDiceValue) =>
        oldDiceValue.map((obj) => (obj.isHeld ? obj : generateNewDie()))
      );
    } else {
      setAttempt(0);
      setTenzies(false);
  ...
  function holdDice(id) {
    setDiceValue((oldDiceValue) =>
      oldDiceValue.map((obj) =>
        obj.id === id ? { ...obj, isHeld: !obj.isHeld } : obj
      )
    );
  }

  const diceElements = diceValue.map((obj) => (
    <Die
      isHeld={obj.isHeld}
      key={obj.id}
      value={obj.value}
  ...
  return (
    <>
      <main>
        ...
        {tenzies && (
          <div className="win">
            Congratulations! You won, and you have taken {attempt} attempt to
            solve it.🥳
          </div>
        )}
